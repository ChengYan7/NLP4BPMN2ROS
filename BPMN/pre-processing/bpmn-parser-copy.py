...
filename = "UC1_CS_baseplate_subassembly_v001.bpmn" 
...
def replace(nreplace, str, key):
    ...
    args:
    - nreplace: nth word in a string
    - str: string with the nth word
    - key: replaces the nth word with the word from the dictionary 

    returns:
    - replaced_str: a string with replaced words
    '''


    words=str.split(" ")
    replaced_str = " ".join([words[word_index] if word_index != nreplace else key for word_index in range(len(words))])
    return replaced_str
